...
def process_dataframe_and_generate_excel(raw_input_df):
    ...
    df_sec1 = standardize_df(valid_raw)
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for r in range(5, tot_row_ct + 1):
            is_header = r in (5, r_sec2_title, tot_row_ct)
            for c in range(1, 20):
        ...
        summary_rows = [
            ...
            [("U", "Nhân công lắp đặt", font_regular, align_right, None),
             ...
             ("Z", f"=IF((Y{r_nc}-X{r_nc})/Y{r_nc})", font_bold, align_right, num_format_percent), # Margin = (Giá bán - Cost) / Giá bán
            ...
            [("U", "Vận chuyển", font_regular, align_right, None),
             ...
             ("Z", f"=IF((Y{r_vc}-X{r_vc})/Y{r_vc})", font_regular, align_right, num_format_percent),
            ...
            [("U", "", font_regular, align_right, None),
             ...
             ("Z", f"=IF((Y{r_ld}-X{r_ld})/Y{r_ld})", font_regular, align_right, num_format_percent),
            ...
            [("U", "Di chuyển (vé xe, xe cty, xăng ...)", font_regular, align_right, None),
             ...
             ("Z", f"=IF((Y{r_dc}-X{r_dc})/Y{r_dc})", font_regular, align_right, num_format_percent),
            ...
            [("U", "Thuê chỗ ở", font_regular, align_right, None),
             ...
             ("Y", 0, font_regular, align_right, num_format_vnd),
             ("Z", f"=IF((Y{r_to}-X{r_to})/Y{r_to})", font_regular, align_right, num_format_percent),
        ...
        ws_kh[f"I{tot_row_ct}"] = f"=I5+I{r_sec2_title}"
        ...
        end_r_pakd = start_r_pakd + n_pakd_items - 1
...
# --- 6.1. HÀM TẠO FILE EXCEL DÙNG CHO NHẬP TRỰC TIẾP ---
def generate_direct_input_excel(raw_input_df):
    ...
    for i in range(n_rows):
        ...
        supplier = row.get("NCC") or row.get("Nhà cung cấp") or ""
        ...
        ws_bg.cell(row=r, column=5, value=desc).alignment = align_left
        ws_bg.cell(row=r, column=6, value=unit).alignment = align_center
        ws_bg.cell(row=r, column=7, value=qty).alignment = align_center

        ws_bg.cell(
            row=r, column=8, value=f"=ROUNDUP(K{r}/(1-J{r}),-3)"
        ).number_format = num_format_vnd

        ws_bg.cell(
            row=r, column=9, value=f"=G{r}*H{r}"
        ).number_format = num_format_vnd

        ws_bg.cell(row=r, column=10, value=margin).number_format = (
            num_format_percent
        )

        ws_bg.cell(row=r, column=11, value=cost).number_format = (
            num_format_vnd
        )

        ws_bg.cell(
            row=r, column=12, value=f"=G{r}*K{r}"
        ).number_format = num_format_vnd

        ws_bg.cell(row=r, column=13, value=supplier).alignment = align_left
    ...
    ws_bg.merge_cells(
        start_row=r_subtotal, start_column=1, end_row=r_subtotal, end_column=8
    )
    ...
    val_sub = ws_bg.cell(
        ...
        column=9,
        value=(
            f"=SUM(I{start_row}:I{end_device_row})" if n_rows > 0 else 0
        ),
    )
    ...
    ws_bg.merge_cells(
        start_row=r_vat, start_column=1, end_row=r_vat, end_column=8
    )

    val_vat = ws_bg.cell(row=r_vat, column=9, value=f"=I{r_subtotal}*8%")
    ...
    ws_bg.merge_cells(
        start_row=r_total, start_column=1, end_row=r_total, end_column=8
    )

    val_tot = ws_bg.cell(
        row=r_total, column=9, value=f"=I{r_subtotal}+I{r_vat}"
    )
    ...
    col_widths_bg = {
        "A": 6, "B": 35, "C": 13, "D": 13, "E": 15, "F": 5,
        "G": 9, "H": 12, "I": 12, "J": 10, "K": 11, "L": 11, "M": 10,
    }
    for col_letter, width in col_widths_bg.items():
        ws_bg.column_dimensions[col_letter].width = width
